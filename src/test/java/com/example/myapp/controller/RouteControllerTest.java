...
public class RouteControllerTest {
    ...
    // the test is currently under construction due to changes in the controller class
//    @Test
//    public void testSaveRouteToUserDevice() throws Exception {
//        // Mock the data that would be generated by calculateRoute method
//        List<String> mockRoute = Arrays.asList("Address 1", "Address 2", "Address 3");
...
//        when(addressService.getAddresses()).thenReturn(addresses);
//
//        // Create a spy of the actual controller
//        RouteController routeController = spy(new RouteController("googleApiUrl", "googleApiKey"));
//
//        // Mock the calculateRoute method to set the route and distance directly
//        doAnswer(invocation -> {
//            Model model = invocation.getArgument(0);
//            model.addAttribute("route", mockRoute);
//            model.addAttribute("distance", mockDistance + " km");
//            return "route";
//        }).when(routeController).calculateRoute(ArgumentMatchers.any(Model.class));
//
//        mockMvc.perform(get("/api/courier/file"))
//                .andExpect(status().isOk());
//
//        verify(response).setContentType("text/plain");
//        verify(response).setHeader("Content-Disposition", "attachment; filename=\"route.txt\"");
//    }
}